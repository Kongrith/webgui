var ROS2D = (function (exports, createjs, ROSLIB) {
	...
	class Viewer {
		...
		constructor(options) {
			...
			this.canvas = canvas
			// add event listener to capture mouse click position
			this.canvas.addEventListener('click', this.handleClick.bind(this));

			this.arrows = [];
			this.goals = [];
			this.mapClickPosition = {x: 0, y:0};
		 ...
		 */
		scaleToDimensions(width, height, zoom_factor) {
			...
			// save scene scaling
			this.scene.scaleX = (width) * zoom_factor;
			this.scene.scaleY = (height) * zoom_factor;

			console.log('Current Scale (x: ' + this.scene.scaleX + ', y: ' + this.scene.scaleY + ')');
		};
		...
		shift(x, y) {
			...
			console.log('Current Shift (x: ' + this.scene.x + ', y: ' + this.scene.y + ')');
		};

		addRobot(x, y, length, direction, color) {
			const arrow = new createjs.Shape();
			const g = arrow.graphics;

			g.beginFill(color).moveTo(0, 0).lineTo(0, -length / 3).lineTo(-length, 0).lineTo(0, length / 3).closePath();
		...
		addCurrentGoal(x, y, length,color) {
			const goal = new createjs.Shape();
			const g = goal.graphics;

			g.beginFill(color).drawCircle(0, 0, length / 2).endFill();
		...
		updateArrowRotation(index, rotation) {
			const arrow = this.arrows[index];
			if (arrow) {
		...
		zoomIn(factor) {
			this.scene.scaleX *= factor;
			this.scene.scaleY *= factor;
			console.log('Current Scale (x: ' + this.scene.scaleX + ', y: ' + this.scene.scaleY + ')');
		}
		...
		updateMapClickPosition(x, y) {
			this.mapClickPosition = {
				x: ((x - this.scene.x)/(this.scene.scaleX)).toFixed(3),
				y: (-1*(y - this.scene.y)/(this.scene.scaleY)).toFixed(3)
			}

			//console.log(this.mapClickPosition)
		}

		getMapClickPosition(){

			return this.mapClickPosition;
		}
		...
		handleClick(event) {
			...
			// Log or use the click position as needed
			console.log('Clicked at position (x: ' + x + ', y: ' + y + ')');

			this.updateMapClickPosition(x, y)
		}
